...
//        echo $this->db->last_query();
    ...
    function get_info($consulta_id) {
        ...
        $this->db->join('paciente', 'paciente.persona_id = consulta.paciente_id');
        $this->db->join('persona', 'persona.persona_id = paciente.persona_id');
        $this->db->where('consulta.id', $consulta_id);
        $this->db->where('consulta.deleted', 0);
        $query = $this->db->get();
        ...
        if ($query->num_rows() == 1) {
            ...
            $fields = $this->db->list_fields('consulta');
    ...
    function save(&$data, $consulta_id = false) {
        ...
        try {
                ...
                if (!$consulta_id or !$this->exists($consulta_id)) {
                    $success = $this->db->insert('consulta', $data);
                    $data['id'] = $this->db->insert_id();
                } else {
                    $this->db->where('id', $consulta_id);
                    $success = $this->db->update('consulta', $data);
                }
